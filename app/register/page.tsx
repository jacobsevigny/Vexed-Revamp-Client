...
export default function RegisterPage() {
  ...
  const handleEmailChange = (email: string) => {
    ...
    setValidations((v) => ({ ...v, emailValid: emailRegex.test(email) }))
  }

  const handleUsernameChange = (username: string) => {
    setFormData((f) => ({ ...f, username }))
    setValidations((v) => ({ ...v, usernameValid: username.trim().length >= 3 }))
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!validations.emailValid) { setError("Please enter a valid email address"); return }
    if (!validations.usernameValid) { setError("Username must be at least 3 characters"); return }
    if (!validations.passwordStrong) { setError("Password must be at least 8 characters"); return }
    if (!validations.passwordsMatch) { setError("Passwords do not match"); return }
    if (!formData.agreeToTerms) { setError("You must agree to the Terms of Service and Privacy Policy"); return }
        ...
        <Link href="/" className="flex justify-center mb-8 hover:opacity-90 transition-opacity">
          <svg className="h-16 w-auto" viewBox="0 0 4106.94 1181.83" xmlns="http://www.w3.org/2000/svg">
            ...
            <g>
              ...
              <g>
                <path className="cls-5" d="M804.1,909.59c54.07-11.81,107.27-26.45,158.84-43.77l-130.34,300.86h-219.13l-94.42-217.97c91.51-4.62,187.17-17.73,285.05-39.12Z" />
                <path className="cls-5" d="M1392.3,15.15l-135.3,171.83-47.4,109.42-2.1,4.84-39.02,90.1-130.24,300.63c-80.17,37.2-171.16,67.43-263.91,87.68-107.86,23.54-215.97,36.08-312.78,36.31l-111.89-258.3h-.02l-40.43-93.39-120.13-277.29L53.78,15.15l428.77,147.28,170.96,347.07c-113.24-99.76-336.25-226.51-336.25-226.51l146.74,180c-33.27,95.46,23.46,205.12,127.1,187.12,68.37-11.88,84.27-59.4,87.19-90.32l44.75,90.87,44.75-90.87c2.91,30.92,18.82,78.44,87.19,90.32,103.64,18.01,160.37-91.66,127.1-187.12l146.74-180s-223,126.75-336.25,226.51l170.96-347.07L1392.3,15.15Z" />
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
                ...
                {formData.password && (
                  <p className="text-xs text-muted-foreground flex items-center gap-1">
                    {validations.passwordStrong
                      ? <><CheckCircle2 className="h-3 w-3 text-green-500" /> Strong password</>
                      : <><XCircle className="h-3 w-3 text-destructive" /> At least 8 characters</>}
                  </p>
                )}
              </div>
                ...
                {formData.confirmPassword && (
                  <p className="text-xs text-muted-foreground flex items-center gap-1">
                    {validations.passwordsMatch
                      ? <><CheckCircle2 className="h-3 w-3 text-green-500" /> Passwords match</>
                      : <><XCircle className="h-3 w-3 text-destructive" /> Passwords do not match</>}
                  </p>
                )}
              </div>
              ...
              <div className="flex items-start gap-2 pt-2">
                <Checkbox
                  ...
                  onCheckedChange={(checked) => setFormData((f) => ({ ...f, agreeToTerms: checked as boolean }))}
                  className="mt-1"
                />
